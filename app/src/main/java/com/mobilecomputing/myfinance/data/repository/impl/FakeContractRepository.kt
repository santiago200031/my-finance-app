...
class FakeContractRepository : ContractRepository {
            ...
            MutableStateFlow<List<Contract>>(
                    listOf(
                            ...
                            Contract(
                                    ...
                                    amount = 45.00,
                                    paymentCycle = PaymentCycle.MONTHLY,
                                    startDate = localDateToDate(LocalDate.now().minusMonths(12)),
                                    nextPaymentDate = localDateToDate(LocalDate.now().plusDays(1)),
                                    endDate = localDateToDate(LocalDate.now().plusDays(1)),
                                    autoRenewEnabled = false,
                                    status = ContractStatus.ACTIVE,
                                    type = ContractType.EXPENSE
                            ),
                            // 3. Expired Test: Expired yesterday, check status update to EXPIRED
                            Contract(
                                    id = "3",
                                    ...
                                    nextPaymentDate = localDateToDate(LocalDate.now().minusDays(1)),
                                    endDate = localDateToDate(LocalDate.now().minusDays(1)),
                                    autoRenewEnabled = false,
                                    status =
                                            ContractStatus
                                                    .ACTIVE, // Logic should flip this to EXPIRED
                                    type = ContractType.EXPENSE
                            ),
                            // 4. Cancelled
                            Contract(
    ...
    override suspend fun updateContract(contract: Contract) {
        _contracts.update { list -> list.map { if (it.id == contract.id) contract else it } }
    }
